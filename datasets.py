...
class COCOClsDataset(Dataset):
    def __init__(self, img_name_list_path, coco_root, label_file_path, train=True, transform=None, gen_attn=False):
        if train or gen_attn:
            img_name_list_path = os.path.join(img_name_list_path,"train_id.txt")
        else:
            img_name_list_path = os.path.join(img_name_list_path,"val_id.txt")
    ...
    def __getitem__(self, idx):
        ...
        if self.train or self.gen_attn :
            img = Image.open(os.path.join(self.coco_root, 'train2014', name + '.jpg')).convert("RGB")
        else:
            img = Image.open(os.path.join(self.coco_root, 'val2014', name + '.jpg')).convert("RGB")
...
class VOC12Dataset(Dataset):
    def __init__(self, img_name_list_path, voc12_root, train=True, crop_size=512, transform=None, gen_attn=False, rescale_range=[0.5, 2.0], ignore_index=255):
    ...
    def __getitem__(self, idx):
        name = self.img_name_list[idx]
        img = Image.open(os.path.join(self.voc12_root, 'JPEGImages', name + '.jpg')).convert("RGB")
        label = torch.from_numpy(self.label_list[idx])
        image = np.asarray(img)
        if self.transform:
            img = self.transform(img)
        img_box = self.__transforms(image=image)
...
def build_transform(is_train, args):
    ...
    if is_train:
        ...
        if not resize_im:
            ...
            # RandomCrop
            transform.transforms[0] = T.RandomCrop(
