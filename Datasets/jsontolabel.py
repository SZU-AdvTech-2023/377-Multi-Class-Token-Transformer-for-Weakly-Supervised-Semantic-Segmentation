...
def cocojson2png(coco_dir, json_path='annotations/instances_train2014.json', cls_type='train_label_2014', save_dir='Dataset/MSCOCO/train_label_2014'):
    save_path = os.path.join(save_dir, cls_type)
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    #annotation_file = os.path.join(coco_dir, 'annotations', json_path)
    with open(json_path, 'r', encoding='utf-8') as annf:
...
if __name__ == '__main__':
    coco_dir = "coco/"
    cocojson2png(coco_dir, json_path='annotations/instances_train2014.json')
